...
from .location import ZFLocation
from .config import base_id, open_file_apworld_compatible
from .item import local_id_to_zf_item, ZFItem

from .zfactor_randomizer.game import Game as CrGame
from .zfactor_randomizer.ips import patch as ips_patch
...
def offset_from_symbol(symbol: str) -> int:
    ...
    if _symbols is None:
        ...
        json_path = path.joinpath("data", "ap_zf_patch", "sm-basepatch-symbols.json")
...
NUM_ITEMS_WITH_ICONS = len(local_id_to_zf_item)
...
class ItemRomData:
    ...
    def register(self, loc: Location) -> None:
        ...
        if loc.player == self.player:
            # my location
            assert isinstance(loc, ZFLocation)
            if not loc.item:
...
def get_multi_patch_path() -> Path:
    ...
    path = Path(__file__).parent.resolve()
    return path.joinpath("data", "ap_zf_patch", "multiworld-basepatch.ips")
