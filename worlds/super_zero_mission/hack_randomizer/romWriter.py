...
class RomWriter:
    ...
    def readBytes(self, address: int, size: int) -> bytes:
        read_bytes = self.rom_data[address:address+size]
        return read_bytes
    ...
    def patch_if_vanilla(self) -> None:
        ...
        if len(self.rom_data) != 4194304:  # redesign !?!?
            if len(self.rom_data) == 3145728:  # vanilla SM
                ...
                with open(patch_path.joinpath('SMCR_uh.ips'), 'rb') as file:
                ...
                patch_path = pathlib.Path(__file__).parent.resolve()
                with open(patch_path.joinpath('Patches/Cliff818052patch.ips'), 'rb') as file:
                    patch_data = file.read()
                self.rom_data = patch(self.rom_data, patch_data)'''

                ips_patches = (
                    'Patches/MSZ ver2.3_Headerless.IPS',
                ...
                assert len(self.rom_data) == 3637248, f"patch made file {len(self.rom_data)}" #SZM
                ...
                # AssertionError: patch made file 3309568
                
            else:
                raise ValueError(f"invalid rom {len(self.rom_data)} - need vanilla SM, unheadered")
